...
<head>
    <meta charset="UTF-8">
    <title>Konten Mengelola Berat</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h2>Konten untuk Mengelola Berat</h2>
        ...
        <div class="card {{ $allowAccess ? '' : 'disabled-card' }}">
            <div class="card-body">
                ...
                @if ($selesai)
                <p style="color: green; font-weight: bold; margin-top: 10px;">✅ Sudah Selesai</p>
                @else
                <form action="{{ route('progress.complete', ['taskId' => $task->id]) }}" method="POST" class="mt-2">
                    @csrf
        ...
        <div class="button-container">
            ...
            <form action="{{ route('task_berat.reset') }}" method="POST" style="display:inline-block;">
                @csrf
                <button type="submit" class="btn-reset" onclick="return confirm('Apakah kamu yakin ingin mengulang semua task berat?')">
                    🔄 Mulai Ulang Task Berat
                </button>
            </form>
        </div>
        @else
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        Swal.fire({
            title: '🎉 Selamat!',
            text: 'Kamu telah menyelesaikan semua task mengelola berat. Hebat banget!',
            icon: 'success',
