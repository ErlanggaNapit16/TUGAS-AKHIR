...
<head>
    <meta charset="UTF-8">
    <title>Konten Mengelola Kecemasan</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h2>Konten untuk Mengelola Kecemasan</h2>
        ...
        <div class="card {{ $allowAccess ? '' : 'disabled-card' }}">
            <div class="card-body">
                ...
                @else
                <form action="{{ route('progress.complete', ['taskId' => $task->id]) }}" method="POST">
                    @csrf
        ...
        <div style="text-align: center;">
            ...
            <form action="{{ route('task_cemas.reset') }}" method="POST" style="display:inline-block;">
                @csrf
                <button type="submit" class="btn-reset" onclick="return confirm('Apakah kamu yakin ingin mengulang semua task cemas?')">
                    🔄 Mulai Ulang Task Cemas
                </button>
            </form>
        </div>
        @else
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        Swal.fire({
            title: '🎉 Selamat!',
            text: 'Kamu telah menyelesaikan semua task mengelola kecemasan. Hebat banget!',
            icon: 'success',
