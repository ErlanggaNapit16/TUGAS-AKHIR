...
<body>
    ...
    <aside id="sidebar" class="sidebar">
        ...
        <ul class="sidebar-nav" id="sidebar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link {{ request()->is('jadwal/konselor') ? 'active' : '' }}" href="{{ route('jadwal.konselor') }}">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                @else
                                                <span class="badge bg-info">Reschedule</span>
                                                @endif
                                            </td>
                                            <td>
                                                @if($schedule->status == 'requested')
                                                <div class="d-flex gap-2">
                                                    <form action="{{ route('jadwal.approve', $schedule->id) }}" method="POST">
                                                        @csrf
                                                        <button type="submit" class="btn btn-success btn-sm">Approve</button>
                                                    </form>
                                                    <form action="{{ route('jadwal.reschedule', $schedule->id) }}" method="POST">
                                                        @csrf
                                                        <button type="submit" class="btn btn-warning btn-sm">Reschedule</button>
                                                    </form>
                                                    <!-- Button trigger modal -->
                                                    <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#rejectModal{{ $schedule->id }}">
                                                        Tolak
                                                    </button>
                                                </div>

                                                <!-- Modal -->
                                                <div class="modal fade" id="rejectModal{{ $schedule->id }}" tabindex="-1" aria-labelledby="rejectModalLabel{{ $schedule->id }}" aria-hidden="true">
                                                    <div class="modal-dialog">
                                                        <div class="modal-content">
                                                            <form action="{{ route('jadwal.reject', $schedule->id) }}" method="POST">
                                                                @csrf
                                                                <div class="modal-header">
                                                                    <h5 class="modal-title" id="rejectModalLabel{{ $schedule->id }}">Alasan Penolakan</h5>
                                                ...
                                                @else
                                                <em class="text-muted">Tindakan sudah dilakukan</em>
                                                @endif
                                            </td>
